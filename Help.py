...
st.markdown('The Cancer Genome Atlas (TCGA) project provides RNAsequencing (RNA-Seq) data encompassing 9,795 tumor samples, spanning 33 distinct cancer types, alongside 727 samples corresponding to adjacent normal tissue, referred here as control. To solve the inherent imbalance in sample sizes between tumor and control datasets, which compromise statistical significance in differential analyses, the inclusion of RNA-Seq data from 8,295 control samples sourced from the Genotype-Tissue Expression (GTEx) project were included (Figure 1). This addition ensures a comprehensive and representative dataset, mitigating the potential biases and fortifying the robustness of subsequent statistical analyses.')
#IMAGE 1
st.image('Figure 1.png', caption="__Figure 1.__ Sample classification. (__A__) Subdivision of TCGA tumors in sample group according to phenotype and inclusion of GTEx tissues included as
control samples in each TCGA tumor. (__B__) Number of samples for each GTEx tissue.", use_column_width=True)
...
st.markdown('Raw data files underwent pre-processing (Figure 2). Additionally, the expression files from TCGA and GTEx were combined, retaining only the genes located in the plasma membrane (GO: 0005886), as CARs target tumor cells’ surface antigens. P-values were calculated with the Mann-Whitney U test and multiple test correction was applied when required to calculate the adjusted p-value.')
